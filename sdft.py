...
class positive_oSDFT():
    ...
    def process_point(self, x):
        ...
        mul = c * self.internal
        y = np.sum(mul.array.real[1:self.size//2])
        y += mul.array[0].real/2
        y += mul.array[self.size//2].real/2
        new = self.internal + (g * (x - y*2)) * (1 / self.size)
        new = fix_array(new)
        self.internal = new
...
pososdft = positive_oSDFT(windowsize)
...
res_pososdft = np.zeros((len(x), windowsize), dtype=np.csingle)
...
for i, point in enumerate(x):
    ...
    res_pososdft[i] = pososdft.process_point(point)
...
dif_pososdft = np.mean(np.abs(res_movdft_f64 - res_pososdft), axis=1)
